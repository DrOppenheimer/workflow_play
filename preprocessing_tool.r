preprocessing_tool <- function(
                                  ...
                                  pseudo_count          = 0.1, # has to be integer for DESeq
...
import_metadata <- function(group_table, group_column, sample_names){
  ...
  export(metadata_matrix)
}
# additional export command to send metadata back to R workspace:
...
DESeq_norm_data <- function (x, regression_filename, pseudo_count,
  ...
  if( identical(DESeq_method,"blind") ){
    ...
    my_metadata <- import_metadata(DESeq_metadata_table, DESeq_metadata_column, sample_names)
